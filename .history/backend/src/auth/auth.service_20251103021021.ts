...
export class AuthService {
  ...
  async validateUser(email: string, password: string) {
    ...
    // Debug info (dev only) - do not log plaintext passwords in production
    // eslint-disable-next-line no-console
    console.debug('[AuthService] validateUser found user:', { id: u._id?.toString?.(), email: u.email });
    if (!u.password) {
      // eslint-disable-next-line no-console
      console.warn('[AuthService] user has no password stored', { email: u.email });
      return null;
    }

    let match = false;
    try {
      match = await bcrypt.compare(password, u.password as string);
    } catch (err) {
      // eslint-disable-next-line no-console
    ...
    // eslint-disable-next-line no-console
    console.debug('[AuthService] bcrypt.compare result for', u.email, ':', match);
