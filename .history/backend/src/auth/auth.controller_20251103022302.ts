...
export class AuthController {
  ...
  async login(@Body() body: LoginDto, @Req() req: any) {
    ...
    // Log tokens returned by service (dev only)
    // eslint-disable-next-line no-console
    console.debug('[AuthController] tokens returned:', {
      accessToken: result?.accessToken ? `${result.accessToken.slice(0, 20)}...` : null,
      refreshToken: result?.refreshToken ? `${result.refreshToken.slice(0, 20)}...` : null,
    });
    return result;
  }
